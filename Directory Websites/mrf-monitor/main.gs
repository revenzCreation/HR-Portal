  ...
  const APPLICANT_HEADERS = [
    'Timestamp', 'Applicant Name', 'Applicant Email', 'Phone', 'Position Applied',
    'Department', 'Source', 'LinkedIn', 'Notes', 'Status'
  ];
  ...
  function doGet(e) {
    try {
      ...
      if (action === 'applicants-list') {
        return json({ ok: true, records: readApplicants() });
      }
  ...
  function doPost(e) {
    try {
      ...
      if (body.action === 'saveApplicant' || body.action === 'applicant-save') {
        return json({ ok: true, record: saveApplicant(body.applicant || body.record || body) });
      }
  ...
  function findSheetByAliases(spreadsheet, aliases) {
    for (const alias of aliases || []) {
      if (!alias) continue;
      const sheet = spreadsheet.getSheetByName(alias);
  ...
  function saveApplicant(input) {
    ...
    const row = [
      new Date(),
      data.fullName || data.applicantName || '',
      data.email || data.applicantEmail || '',
      data.phone || '',
      data.position || data.positionApplied || '',
      data.department || '',
      data.source || '',
      data.linkedin || '',
      data.notes || '',
